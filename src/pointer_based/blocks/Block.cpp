...
int Block::rank(int c, int i) { return 0; }

int Block::select(int c, int j) { return -1; }

int64_t Block::length() const { return end_index_ - start_index_ + 1; }

std::string Block::represented_string() { return source_.substr(start_index_, length()); }

std::vector<Block *> &Block::children(int leaf_length, int r) { return children_; }

void Block::clean_unnecessary_expansions() {}

bool Block::is_leaf() { return true; }

int Block::access(int i) { return -1; }
